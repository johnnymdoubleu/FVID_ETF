...
class Scrapper(object):
    ...
    def open38(self):
        try :
            self.driver.get("http://www.38.co.kr/html/fund/index.htm?o=nw&page=1")
            self.driver.execute_script("window.open();")
            self.driver.switch_to_window(self.driver.window_handles[1])
            self.driver.get("https://kr.investing.com/ipo-calendar/")
            self.driver.switch_to_window(self.driver.window_handles[0])
        except :
            print("404 TimeoutException!")
            exit()
    ...
    def getIPO(self):
        try :
            ...
            ip = table.find_all('font', color=True)
            ipo = list()

            for i in ip:
                i = 'u'+ i.text
                if any([re.search(u'[\u3131-\ucb4c]', i)]):
                    i = i.replace("u","")
                    if i!="(유가)":
                        if "호" not in i :
                            i = i.replace("(유가)","")
    ...
    def searchNaver(self, ipo):
        try :
            self.driver.execute_script("window.open();")
            self.driver.switch_to_window(self.driver.window_handles[2])
            self.driver.get("https://finance.naver.com/")

            engine = self.wait.until(EC.presence_of_element_located((By.ID, "stock_items")))
            unique = list()
            for i in ipo:
                engine.send_keys(i)
                engine.send_keys(Keys.DOWN)
                ...
                try :
                    ...
                    unique.append(code)
                    engine = self.driver.find_element_by_id('stock_items')
                except :
                    engine = self.driver.find_element_by_id('stock_items')
                    code = "Needs Manual"
                    unique.append(code)
    ...
    def nextPage(self) :
        ...
        # daum = self.driver.find_elements_by_xpath("//a[contains(text(), '[다음]')]")
        my_element = self.driver.find_element_by_xpath("//a[text()='[다음]']")
        my_element.click()
        return self.driver.current_url[-1]
